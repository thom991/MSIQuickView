function rotationGUI(i)
    %# read image
    global original_pts modified_pts x y I I2 J
    x = [];
    y = [];
    ...
    I = load(strcat('C:\threed_trial_images\Part',num2str(i-1),'_modified.mat'));        
    I = I.J;    
    end
    figure(1);
    subplot(2,1,1); imagesc(I);
    I2 = load(strcat('C:\threed_trial_images\Part',num2str(i),'.mat'));
    I2 = I2.B;
    subplot(2,1,2); imagesc(I2);
%     colormap(strcat('hot','(32)'));
    [x,y] = ginput;
    x_org = x(1:2:end);
    x_mod = x(2:2:end);
    y_org = y(1:2:end);
