function [uz4, store_size] = find_closest_number_to_x_from_a_list_class_hdf(number_of_scans, unique_list_hdf_final, filename, isos_dir)
%FIND closest number to x from a list for classification using hdf files.
...
save(strcat('new_mz_list_',num2str(file_no),'_isos.mat'),'list_mz2')
...
% toc
%%
%Create fake Matrix size (mz vals X total number of scans)
matrix_start = zeros(size(uz4,2),sum(max_num(:)),'single');
store_size = size(matrix_start);
% fprintf('Begin Step 3 of 5.... \n')
% col_no = 1;    
% %%
% % tic
% for file_no = 1:number_of_scans%:15
...
%             end
%         end 
% % number_of_point_count_per_scan = csvread(strcat('Brain 3-',num2str(file_no),'_isos.csv'),1,0);
% % if file_no < 10    
% number_of_point_count_per_scan = csvread(fake_name2,1,0);
% % else
% % number_of_point_count_per_scan = csvread(fake_name2,1,0);    
% % end
% int_val_isos = number_of_point_count_per_scan(:,3);
% mz_vals_saved = load(strcat('new_mz_list_',num2str(file_no),'_isos.mat'));
% mz_vals_saved = mz_vals_saved.list_mz2;
% % list_mz = (number_of_point_count_per_scan(:,4));
% % for new_list = 1:size(list_mz,1)
% % [c index] = min(abs(unique_list_hdf_final-list_mz(new_list)));
% % closestValues = unique_list_hdf_final(index); % Finds first one only!
% % list_mz(new_list) = closestValues;
% % end
% number_of_point_count_per_scan2 = number_of_point_count_per_scan(:,1);
% parfor num_of_each_scans = 1:max_num(count_file_no)
%    temp1 = find(number_of_point_count_per_scan2 == num_of_each_scans); 
%    vals_for_each_scan(num_of_each_scans) = size(temp1,1); 
% end
% count_file_no = count_file_no + 1;
% for no_of_mz_vals_from_list_length = 1: size(vals_for_each_scan,2)%(num_of_each_scans)
%     if no_of_mz_vals_from_list_length > 1
%     start_point = sum(vals_for_each_scan(1:no_of_mz_vals_from_list_length-1)) + 1;
%     else
%     start_point = 1;
%     end
%     %disp('start_point=='); disp(start_point); disp(':'); disp((start_point + vals_for_each_scan(count_temp)-1));
%     
%     for no_of_mz_vals_from_list = start_point: (start_point + vals_for_each_scan(count_temp)-1)
%     index = find(uz4 == mz_vals_saved(no_of_mz_vals_from_list));
%     matrix_start(index, col_no) = int_val_isos(no_of_mz_vals_from_list);
%     end
%     col_no = col_no + 1;
...
% end
% % toc
% % save matrix_new.mat matrix_start uz4
% delete(strcat(isos_dir,'/*.mat'))
cd(current_dir)