function [no_of_intensity_files] = open_all_mz_and_produce_sum_of_intensities(range_selected, k2, number_of_scans, uz4, imp_mass2, k_start_lda, k_last_lda)
%eg;
...
global filename size_sum_of_intensities count_2 normalize_data_mode ll O_was_present lower_limit_mz_value upper_limit_mz_value single_range_value double_range_value final_mz_val pathname
...
if exist(strcat(pathname,saveTempFilesToFolder),'dir')
    if size(dir(strcat(pathname,saveTempFilesToFolder)),1) > 2
    rmdir(strcat(pathname,saveTempFilesToFolder),'s')
%     cd('C:\');
    mkdir(strcat(pathname,saveTempFilesToFolder));
    end
else
%     cd('C:\');
    mkdir(strcat(pathname,saveTempFilesToFolder));    
...
%    'Pick the Excel file'); 
% %
% opening_xls_sheet = xlsread(strcat(open_excel_pathname,'\',open_excel_filename));
...
k = k2;
%%
% if isempty(imp_mass2)
if isempty(k_last_lda)
  final_mz_val = uz4(k_start_lda:end);  
else
final_mz_val = uz4(k_start_lda:k_last_lda);
end
[no_of_intensity_files] = size(final_mz_val,2);
% else
% final_mz_val = imp_mass2;    
% end
fake_name = filename;
...
cd(current_dir)
% time_temp = toc; fprintf('time tt is %d',time_temp) 
end                    
                    k_final_2 = count_2;
                    k = k + 1;
                    row_num = row_num + 1;
